...
function openInventoryUse() {
    ...
    if (player.inventory.length === 0 && !player.equipped["무기"] && !player.equipped["방어구"]) {
        addLog("가방", "사용할 아이템이 없습니다.", "log-sys");
        addBtn(panel, "← 마을로", () => renderScene("Town"));
        return;
    }

    const styles = window.RARITY_STYLE || {};
    const show = (item, idx, isEquipped) => {
        const b = document.createElement('button');
        b.className = 'excel-btn';
        const col = styles[item.rarity]?.color || "#333";
        const tag = isEquipped ? '[장착] ' : '';
        b.innerHTML = `<span style="color:${col}">${tag}[${item.rarity}]</span> ${item.name} — <small>${item.desc || ''}</small>`;
        b.onclick = () => {
            if (isEquipped) {
                player.inventory.push(item);
                player.equipped[item.slot] = null;
                addLog("해제", `[${item.name}] 장착 해제`, "log-sys");
            } else if (item.slot === "소비" && item.effect) {
                const result = item.effect(player);
                player.inventory.splice(idx, 1);
                addLog("사용", result || `${item.name} 사용!`, "log-attain");
                checkLevelUp();
                ...
                addLog("장착", `[${item.name}] 장착 완료!`, "log-attain");
            }
            updateUI();
            openInventoryUse();
        };
    ...
    if (w) show(w, -1, true);
    if (a) show(a, -1, true);
    player.inventory.forEach((item, idx) => show(item, idx, false));
    addBtn(panel, "← 마을로", () => renderScene("Town"));
}
